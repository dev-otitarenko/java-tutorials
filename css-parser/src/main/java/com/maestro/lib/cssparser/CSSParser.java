...
public final class CSSParser {
	 ...
	 */
	public static List<CSSRule> parse(final String css) throws Exception {
	...
	private final List<CSSPropertyValue> values;
	private State state;
	...
	private CSSParser() {
		...
		this.values = new ArrayList<CSSPropertyValue>();
	 ...
	 */
	private void parse(final List<CSSRule> rules, final Character c, final Character nextC) throws Exception {
	...
	private void parseValue(final Character c) throws IncorrectFormatException {
		...
		if (Chars.SEMI_COLON.equals(c)) {
			...
			CSSPropertyValue pv = new CSSPropertyValue(propertyName.trim(), valueName.trim());
			values.add(pv);
	 ...
	 * @throws IncorrectFormatException If any error occurs
	 */
	private void parsePropertyName(final List<CSSRule> rules, final Character c) throws IncorrectFormatException {
		if (Chars.COLON.equals(c)) {
			...
			for (String s : selectorNames) {
				CSSSelector selector = new CSSSelector(s.trim());
				rule.addSelector(selector);
			}
			selectorNames.clear();

			CSSSelector selector = new CSSSelector(selectorName.trim());
			selectorName = "";
			rule.addSelector(selector);

			// Add the property values
			for (CSSPropertyValue pv : values) {
