...
public class XMLParam implements IXmlParam {
    ...
    public String getDefaultValue (final PZDocInput inp) {
        String ret = "";
        if (inp != null) {
            if (inp.getField().equalsIgnoreCase("HNUM") && 
                (_doc.getCDoc().startsWith("F13") || _doc.getCDoc().startsWith("J13")) &&
                _cnt4Curdate > 0) {
                ret = String.valueOf(_cnt4Curdate);
            }
            
            if (_doc.getCDocStan() == 1 && inp.getField().equals("HZ")) { // Звітня
                ret = "1";
            } else if (_doc.getCDocStan() == 2 && inp.getField().equals("HZN")) { // Нова звітня
                ret = "1";
            } else if (_doc.getCDocStan() == 3 && inp.getField().equals("HZU")) { // Уточнююча
                ret = "1";
            }
                // Дата заполнения
...
//                    ret = Utils.isEmpty(this._pay.getCTerrit()) ? "00" : this._pay.getCTerrit().substring(0, 2);
//            }
//            if (inp.getField().equalsIgnoreCase("RAY") && this._doc.getCDoc().startsWith("S")) {
//                    ret = Utils.isEmpty(this._pay.getCTerrit()) ? "000" : this._pay.getCTerrit().substring(2, 5);
//            }
